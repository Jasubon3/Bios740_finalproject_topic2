...
def check_version(config, model_class, model_path):
    if os.path.exists(model_path):
        model_file = model_path if model_path.endswith('.bin') else os.path.join(model_path, 'pytorch_model.bin')

        # Skip legacy version check if old-style .bin file is not present
        if not os.path.exists(model_file):
            print(f"Skipping version check; no pytorch_model.bin found at {model_file}")
            return
